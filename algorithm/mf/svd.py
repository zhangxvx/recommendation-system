...
class SVD(IterationEstimator):
    ...
    def _prepare(self):
        ...
        # user factor
        # self.p = np.random.normal(size=(self.user_num, self.n_factors))
        self.p = np.zeros((self.user_num, self.n_factors), dtype=np.double) + 0.1
        
        # item factor
        # self.q = np.random.normal(size=(self.item_num, self.n_factors))
        self.q = np.zeros((self.item_num, self.n_factors), dtype=np.double) + 0.1
    ...
    def _pred(self):
        return self.global_mean + np.repeat(np.asmatrix(self.bu).T, self.item_num, axis=1) + np.repeat(
            np.asmatrix(self.bi), self.user_num, axis=0) + np.dot(self.p, self.q.T)
